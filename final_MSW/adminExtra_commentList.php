...
if (isset($_GET['IID'])) { // note that the spelling 'IID' is based on the query string variable name

	// product id available, vaIIDate the information, then compose a query accordingly to retrieve information.

	$IID = intval($_GET['IID']); // force all input into an integer.  If the input is a string or empty, it will be converted to 0.

	// vaIIDate the product id -- check to see if it is greater than 0
    if ($IID>0 ){
        /* ----------------
            Got a vaIID link id, start composing the content of this page including
            (1) link info:
        ...
        // (1) Get the link info
        $sql = "SELECT AnchorText, URL from Links WHERE IID = ?";
        ...
        //  Get all tag options
        $sql = "SELECT LCID, Comment, `Date`, Display from LinkComment WHERE IID = ? Order by `Date` DESC";
                    ...
                    $commentList = $commentList.
                        "<div class='commentBox'>
                            <div>
                                ...
                                <span class='action'><a href='adminExtra_commentEdit.php?IID=$IID&lcid=$LCID&option=$optionValue'>$optionText</a></span>
                            </div>
        ...
        $output = $output.$commentList;
			...
			// link id <= 0. reset $IID. prepare error message
			$IID = "";
	...
	// $_GET['IID'] is not set, which means that no link id is provided
