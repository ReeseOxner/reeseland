...
Three pieces of info are expected from the URL query string: IID, tid, option (A or R -- add or remove).
...
if (isset($_GET['IID']) && isset($_GET['tid']) && isset($_GET['option'])){
    ...
    if ($_GET['option'] == "R") {
        ...
        $sql = "Delete From LinkTag Where IID = ? AND TagID = ?";

    } else {
        // insert a new link-tag association
        $sql = "Insert Into LinkTag (IID, TagID) Values (?, ?)";
    }

    $stmt = $conn->stmt_init();
    if($stmt->prepare($sql)){

        $stmt->bind_param('ii',$IID, $tid);
        if ($stmt->execute()){
            header("Location: adminExtra_tagList.php?IID=$IID");
            exit;
        } else {
    ...
    // if any of the IID, tid, or option is missing from the URL query string, redirect the user to the admin home page (adminExtra_list.php)
