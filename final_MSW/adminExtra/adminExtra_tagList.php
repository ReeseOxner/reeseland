...
if (isset($_GET['IID'])) { // note that the spelling 'IID' is based on the query string variable name
    ...
    if ($IID>0 ){
        ...
        // Get the tags for this link (based on $IID)
        ...
        foreach ($tagNameArr as $tid => $tagname) {
            ...
            $tagList = $tagList."
            <div class='tagItem'><span class='$tagClass'>$tagname</span> <span class='action'><a href='adminExtra_tagEdit.php?IID=$IID&tid=$tid&option=$optionValue'>$optionAnchorText</a></span></div>
            ";
