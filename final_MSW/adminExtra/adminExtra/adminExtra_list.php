...
// Retrieve the link & category info using a join query
	$sql = "SELECT MSWImages.LinkID, MSWImages.title, MSWImages.pageURL, MSWproductType.CatName FROM MSWImages, MSWproductType where MSWImages.CID = MSWproductType.CID order by MSWproductType.CatName, MSWImages.title";
	...
	if ($stmt->prepare($sql)){
		...
		$stmt->execute();
		$stmt->bind_result($LinkID, $title, $pageURL, $CatName);
		...
		while($stmt->fetch()){
			...
			$tblRows = $tblRows."
                        <tr><td><a href='$pageURL'>$title</a></td>
								 <td>$CatName</td>
